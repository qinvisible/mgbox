<script setup>
    ...
    const removeModal = useModal((user) => {
        router.delete('/user/' + user.user.id);
    });
</script>
<template>
    <div class="relative ps flex  flex-col items-center justify-center" style="height: 100vh">
        <div class="container" style="height: 100vh;">
            <h2 class="text-2xl py-5">User Yang Tersedia</h2>
            <a href="/user/create" class="btn btn-neutral btn-md mb-4">Tambah User</a>
            <div v-if="props.flash"><span class="text-success py-4"> {{ props.flash }} </span></div>
            <div class="h-100 pb-12">
                <table class="table overflow-x-auto border table-zebra text-center text-sm font-dark dark:border-neutral-500 table-pin-rows">
                    <thead class="border-b font-medium dark:border-neutral-50">
                        <tr>
                            ...
                            <th class="border-r px-6 py-4 dark:border-neutral-500 text-lg font-medium">Posisi</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="user in users" :key="user.id" class="border-b transition duration-300 ease-in-out hover:bg-neutral-100 dark:border-neutral-500 dark:hover:bg-neutral-600">
                            <td class="dark:border-neutral-500"><strong>{{ user.user.name }}</strong></td>
                            <td class="dark:border-neutral-500">{{ user.user.email }}</td>
                            <td class="dark:border-neutral-500">{{ user.role.name }}</td>
                            <td class="dark:border-neutral-500">
                                <a :href="'/user/' + user.user.id" class="btn btn-primary btn-sm mr-3" title="Edit Peran">
                                ...
                                <button class="btn btn-secondary btn-sm" @click="removeModal.open(user)">
