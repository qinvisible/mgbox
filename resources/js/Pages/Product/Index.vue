    ...
    <div class="relative ps flex  flex-col items-center justify-center" style="height: 100vh">
        <div class="container" style="height: 100vh;">
            ...
            <div class="add-amountfield relative w-1/2 flex my-3 hidden" id="addAmount">
                <div class="w-1/6 h-100"><div class="h-100 self-center">Jumlah</div></div>
                <div class="w-1/2"><input type="number" name="amount" id="addAmountVal" class="w-100 input input-bordered"></div>
            ...
            <div class="h-3/4 pb-12 overflow-x-auto">
                <table class="table overflow-x-auto border text-sm font-light table-pin-rows table-pin-cols">
                    <thead class="border-b font-medium dark:border-neutral-50">
                        <tr class="w-100">
                            <th class="border-r px-6 py-4  bg-neutral text-neutral-content text-lg w-12">Nama</th>
                            <th class="border-r px-6 py-4  bg-neutral text-neutral-content text-lg w-12">Deskripsi</th>
                    ...
                    <tbody>
                        <tr v-for="product in products" :key="product.id" class="w-100">
                            ...
                            <td class="ws-nw mx-2 relative">
                                {{ product.amount }}
                                <button class="btn btn-xs btn-neutral mx-2" @click="openForm(product.id)">+</button>
                            </td>
                            <td>{{ product.location }}</td>
                            <td>{{ product.category.name }}</td>
                            <td class="ws-nw">
                                <a :href="'/product/' + product.id" class="btn btn-primary btn-sm mr-3" title="Edit Peran">
