<template >
    <div class="relative flex flex-col items-center justify-center overflow-hidden">
        <div class="container">
            ...
            <div v-if="flash"><span class="text-success"> {{ flash }} </span></div>
            <table class="table overflow-x-auto border table-zebra text-center text-sm font-light dark:border-neutral-500">
                ...
                <tbody>
                    <tr v-for="role in roles" :key="role.id" class="border-b transition duration-300 ease-in-out hover:bg-neutral-100 dark:border-neutral-500 dark:hover:bg-neutral-600">

                        <td v-if="role.name != 'Super Admin'">{{ role.name }}</td>
                        <td v-if="role.name != 'Super Admin'">
                            <ul>
                                {{ role.perpermission }}
                                <li v-for="permission in (role.permission)"  :key="permission.id">{{ permission }}</li>
                            </ul>
                        </td>
                        <td v-if="role.name != 'Super Admin'">
                            <a :href="'/role/' + role.id" class="btn btn-primary btn-sm mr-3" title="Edit Peran">
